...
static AVIndexEntry* mov_find_next_sample(AVFormatContext* s, AVStream** st)
{
	...
	for (i = 0; i < s->nb_streams; i++) {
		...
		if (msc->pb && msc->current_sample < avst->internal->nb_index_entries) {
			...
			av_log(s, AV_LOG_INFO, "stream %d, sample %d, pos: %lld, dts %"PRId64"\n", i, msc->current_sample, current_sample->pos, dts);//tmp
...
retry:
	...
	av_log(NULL, AV_LOG_INFO, "next sample, stream: %d, pos: %lld\n", st->index, sample->pos);//tmp
